...
const Step_3 = () => {
  ...
  const handleAddAddOns = (x, box) => {
    let addonType = x
    if (usersAddonReducer.length === 0) {
      //if the addons list is empty insert the clicked addon
      ...
      addOnList.forEach((elem) => {
        if (addonType === elem.addOn) {
          //Confirm if addon selected is already included in the list
          included = true
        }
      })

      if (!included) {
        //If not included then dispatch to be included
        dispatch(addAddOnsAction(addonType))
        if (box === 1) {
          box_1.current.classList.add('active')
          check_1.current.classList.add('active')
      ...
      <section id="step_3">
        ...
        <div className="part_2">
          <div
            className="box"
            onClick={() => {
              handleAddAddOns('Online Service', 1)
            }}
            ref={box_1}
          >
            <div>
          ...
          <div
            className="box"
            onClick={() => {
              handleAddAddOns('Larger Storage', 2)
            }}
            ref={box_2}
          >
            <div>
          ...
          <div
            className="box"
            onClick={() => {
              handleAddAddOns('Customizable Profile', 3)
            }}
            ref={box_3}
          >
            <div>
